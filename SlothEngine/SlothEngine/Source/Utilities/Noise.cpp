...
float slth::Noise::DotGridGradient(int ix, int iy, float x, float y, u32 seed)
{
	...
	Vector2 distanceVec(x - (float)ix, y - (float)iy);

	return distanceVec.DotProduct(distanceVec, gradientVec);
}

float slth::Noise::PerlinNoise(slth::Vector2 vec, u32 seed)
{
	...
	float ix0 = Vector2::Lerp({ n0, n1 }, sx);

	n0 = DotGridGradient(x0, y1, vec.x, vec.y, seed);
	n1= DotGridGradient(x1, y1, vec.x, vec.y, seed);
	float ix1 = Vector2::Lerp({ n0, n1 }, sx);

	float value = Vector2::Lerp({ ix0, ix1 }, sy);

	return value;
}


float slth::Noise::GetPerlinNoise(slth::Vector2 vec, float maxX, float maxY, float noiseInputRange, u32 seed)
{
...
float slth::Noise::GetOctavePerlinNoise(slth::Vector2 vec, float maxX, float maxY, float noiseInputRange, int numOctaves, float persistance, u32 seed)
{
	if (numOctaves <= 0)
		return 0.f;
	...
	for (int i = 0; i < numOctaves; ++i)
	{
		...
		//TODO: see notes in ValueNoise.txt about seeding this
