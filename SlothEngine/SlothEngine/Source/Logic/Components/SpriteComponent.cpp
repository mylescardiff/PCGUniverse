...
bool slth::SpriteComponent::PostInit()
{
	...
	if (pTransformComp)
	{
		//m_textureRect = RectInt(0, 0, (int)pTransformComp->GetSize().x, (int)pTransformComp->GetSize().y);
	}
	else
	{
...
void slth::SpriteComponent::Draw(IGraphics* pGraphics)
{
	...
	pGraphics->DrawTexture(m_path.c_str(), drawRect, m_textureRect, m_direction == FacingDirection::kLeft, pTransformComp->m_rotation, m_origin, m_repeat, 1.f);
