...
namespace slth
{
	...
	bool RigidBodyComponent::PostInit()
	{
		return true;
	}
	...
	void RigidBodyComponent::SetBody(World* pWorld)
	{
		...
		bodyDef.angle = pTransform->m_rotation;
		...
		Vector2 pTransPosition = pTransform->GetPosition();
		b2Vec2 physPosition =
		{
		...
		m_pPhysicsBody->SetTransform(physPosition, pTransform->m_rotation * 0.0174533f);
	...
	void RigidBodyComponent::ApplyLinearImpulse(Vector2 & directionalForce)
	{
	...
	void RigidBodyComponent::Move(Vector2 amount)
	{
	...
	void RigidBodyComponent::SetPosition(Vector2 position)
	{
		auto transform = m_pPhysicsBody->GetTransform();
