...
slth::Actor* slth::ActorFactory::CloneActor(Actor& otherActor, Vector2 position)
{
	return new Actor(otherActor, position, ++m_nextActorId);
	
}

slth::Actor* slth::ActorFactory::CreateBlockActor(const char* tag, Vector2 position, Vector2 size, Color color, std::map<Id32, Actor*>& list)
{
	Actor* pActor = new Actor(m_nextActorId++);
	pActor->SetTag(tag);
