...
public:
	...
	SLTH_API bool IsButtonPressed(MouseButton button) {
		return m_buttonData[button].m_pressed; 
	}

	/** Detect if the specified mouse button is being held down */
	GLUA()
	SLTH_API bool IsButtonDown(MouseButton button) { return m_buttonData[button].m_down; }
...
protected:
	...
	ButtonData m_buttonData[kNumButtons];
