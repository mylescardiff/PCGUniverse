...
using slth::IMouse;
#define _CHECK_BUTTON(button) assert(button >= 0 && button <= kNumButtons)
...
bool slth::IMouse::IsDoubleClicked(MouseButton button)
{
	...
	int timeDifference = m_buttonData[button].m_pressedTimes[0] - m_buttonData[button].m_pressedTimes[1];

	bool result = timeDifference > 0 && timeDifference < kDoubleClickThreshold;
	if (result)
	{
		m_buttonData[button].m_pressedTimes[0] = 0;
		m_buttonData[button].m_pressedTimes[1] = 0;
		m_buttonData[button].m_pressedTimes[2] = 0;
	}
	return result;
...
{
	_CHECK_BUTTON(button);
	int timeDifference = m_buttonData[button].m_pressedTimes[0] - m_buttonData[button].m_pressedTimes[2];
...
void slth::IMouse::SetButton(MouseButton button, bool down)
{
	...
	if (down)
	{
		...
		m_buttonData[button].m_pressedTimes[1] = m_buttonData[button].m_pressedTimes[0];
		m_buttonData[button].m_pressedTimes[2] = m_buttonData[button].m_pressedTimes[1];
		m_buttonData[button].m_pressedTimes[0] = static_cast<u32>(DateTime::TickTimestamp());
	...
	_CHECK_BUTTON(button);
	m_buttonData[button].m_pressed = down;
	if (down)
	{
		m_buttonData[button].m_down = true;
		
	}
	else
	{
		m_buttonData[button].m_down = false;
	}
}
